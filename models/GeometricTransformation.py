...
class GeometricTransformation:
    ...
    def calculateViewUpVector(normalVector, viewUp):
        scalarProduct = (normalVector.x * viewUp.x) + \
            (normalVector.y * viewUp.y) + (normalVector.z * viewUp.z)
        ...
        scalarModule = math.sqrt(
            (pow(scalarVector.x, 2) + pow(scalarVector.y, 2) + pow(scalarVector.z, 2)))
    ...
    def calculateSRTVertexes(vertexes, vertexMinWindow, vertexMaxWindow, vertexMinView, vertexMaxView, dp):
        deltaView = Vertex(vertexMaxView.x - vertexMinView.x,
                           vertexMaxView.y - vertexMinView.y, 1)
        deltaWindow = Vertex(vertexMaxWindow.x - vertexMinWindow.x,
                             vertexMaxWindow.y - vertexMinWindow.y, 1)
